...
public class ConvenientCard implements Payment{
	...
	public ConvenientCard(IDCard theDinhDanh) throws CannotCreateCard{
		...
		if(Year.now().getValue() -  Integer.parseInt(theDinhDanh.getNgayThangNamSinh().substring(6, 10)) < 12 )
			throw new CannotCreateCard("Not enough age");
		else
			this.loaiThe = (Year.now().getValue() - Integer.parseInt(theDinhDanh.getNgayThangNamSinh().substring(6, 10)) <= 18) ? "Student" : "Adult";
